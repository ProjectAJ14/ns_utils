...
/// invoked. The debounced function comes with a
/// [Debounce.cancel] method to cancel
/// delayed `func` invocations and
/// a [Debounce.flush] method to immediately invoke them.
...
class Debounce {
  ...
  Object _invokeFunc(int time) {
    final List<Object> args = _lastArgs;
    final Map<Symbol, Object> namedArgs = _lastNamedArgs;
  ...
  bool _shouldInvoke(int time) {
    final num timeSinceLastCall = time - (_lastCallTime ?? double.nan);
    final int timeSinceLastInvoke = time - _lastInvokeTime;
  ...
  int _remainingWait(int time) {
    final int timeSinceLastCall = time - _lastCallTime;
    final int timeSinceLastInvoke = time - _lastInvokeTime;
    final int timeWaiting = _wait - timeSinceLastCall;
  ...
  Object call(
    ...
    final int time = DateTime.now().millisecondsSinceEpoch;
    final bool isInvoking = _shouldInvoke(time);
...
/// The throttled function comes with a [Throttle.cancel]
